  ...
  server.listen((request) async {
    if (request.uri.path == "/login") {
      ...
      if (_testToken(request)) {
        request.response.write('BEM VINDO');
      } else {
        request.response.write('NEGADO');
      }
      request.response.write('teste');
    } else {
      request.response.statusCode = 404;
      request.response.write('nao encontrada');
    }
...
bool _testToken(HttpRequest request) {
  ...
  return sign64 == signGlobal && DateTime.now().millisecondsSinceEpoch < payload["exp"];
}
...
tecnofarma.vercel.app/authentication
